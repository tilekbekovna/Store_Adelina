...
auth_router = APIRouter(prefix='/auth', tags=['auth nur crud'])
...
async def register(user: UserProfileInputSchema, db: Session = Depends(get_db)):
    ...
    if user_db:
        raise HTTPException(detail='Мындай usrname бар экен', status_code=400)
    if email_db:
    ...
    return {'message': 'сиз регистрация болднуз'}
...
async def logout(refresh_token: str, db: Session = Depends(get_db)):
    ...
    if not stored_token:
        raise HTTPException(status_code=401, detail="маалмат туура эмес")
